...
```json
{"status": "ok", "service": "googletranslate-2api", "version": "2.10.0"}
```
...
| `/v1/admin/keys/probe` | POST | 自检 Key 池有效性（每 Key 一次最小探测，返回哈希+状态） |
...
| `DEMO_MODE` | 否 | `false` | 无 Key 演示模式：不访问真实上游，返回 `demo:` 前缀结果 (v2.10.0) |
