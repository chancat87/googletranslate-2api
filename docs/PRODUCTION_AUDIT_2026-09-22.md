...
| 项目路径 | `/opt/googletranslate-2api`（git tag `v2.12.2`） |
| 运行方式 | Docker Compose 蓝绿生产栈：`gateway`(nginx:8088) + `app-blue`/`app-green` + `redis` |
| 应用镜像 | `ghcr.io/lza6/googletranslate-2api:v2.12.2` |

## 2. 验收清单

| 类别 | 项 | 结果 |
|---|---|---|
| 质量 | 全量回归 | 308 passed / 1 skipped，覆盖率 98.94% |
...
| Deploy | GHCR 构建推送 `v2.12.2` | success |
| 公网 | `GET /health` | 200，`version: 2.12.2` |
| 公网 | `GET /ready` | 200 |
...
- 仓库工作区干净：除用户未跟踪目录外无未提交改动；`v2.12.2` 已提交、推送、发版
...
# 升级（零中断） / 回滚（零中断）
bash deploy/upgrade.sh ghcr.io/lza6/googletranslate-2api:v2.12.2
bash deploy/upgrade.sh ghcr.io/lza6/googletranslate-2api:v2.12.1
