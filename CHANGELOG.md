...
- 出口策略改为：先本机服务器直连；直连 429/网络失败/5xx 再按“最低延迟”逐个换代理，且每换一个都先重试本机
- 代理分配改为低延迟粘滞复用：成功记录并更新延迟 EWMA，不再每请求强行换出口，也不再按使用次数冷却
- 失败代理进入 `PROXY_RETEST_SECONDS`（默认 1s）冷却，到点重新测延迟/可用性，能用即回池
- 移除每日限额（`PROXY_MAX_USE_PER_DAY`）与递增冷却（`PROXY_USE_COOLDOWN_MAP`）
- 校验/成功均回填延迟，`/v1/admin/proxy` 快照新增 `latency_ms`

### 验证

- 新增直连优先/低延迟粘滞/回池冷却测试；代理池 19 项 + 单飞 8 项通过；mypy/ruff 全过
- 生产压测对比见发布说明（预期：本机可用时 QPS 提升，429 触发才切代理）
...
- 走代理时同一坏代理不做内部重试（`attempts=1`），快速交还上层按 Key 换下一个代理
- 新增 `PROXY_MAX_ATTEMPTS`（默认 3）：同一 Key 失败后最多换 N 个代理再切 Key，提升轮换成功率与延迟
- 生产实测免费代理成功率低（详见发布说明），高并发建议住宅/付费代理池；本版改善单次失败时间从最坏 26s 降到连接超时级别

### 验证

- 新增“传输错误自动换代理”测试；26 项代理/单飞测试通过；mypy 0 错误；ruff 全过
