...
async def test_p15_ready_503_when_upstream_down(client):
    ...
    with patch.object(
        app_main.provider.client,
        "post",
        new=AsyncMock(side_effect=httpx.HTTPStatusError("500", request=bad.request, response=bad)),
    ):
        r = await client.get("/ready")
...
async def test_p11_chunked_stream_when_enabled(client, monkeypatch):
    ...
    with patch.object(
        app_main.provider.client, "post", new=AsyncMock(return_value=_mock_upstream_response("句"))
    ):
        r = await client.post(
            "/v1/chat/completions",
            json={
